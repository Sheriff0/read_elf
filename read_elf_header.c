...
extern int read_elf_osabi(struct elf32_hdr *e_hdr, char **buff);
...
read_elf_e_shstrndx(struct elf32_hdr *e_hdr, void *fimg, char **buff)
{
	...
	(buff[l_count++] = e_shstrndx.name, buff[l_count++] =
	 e_shstrndx.pad_start);

	get_mb_elf_value('s', e_hdr->e_ident[EI_DATA], &tmp,
			 &e_hdr->e_shstrndx);
...
read_elf_e_shnum(struct elf32_hdr *e_hdr, void *fimg, char **buff)
{
	...
	shnum = (tmp.s != SHN_UNDEF) ?
	    ...
	    (get_mb_elf_value('i', ei_data, &tmp, &e_hdr->e_shoff),
	     sh_offset = tmp.i,
	     shdr = fimg + sh_offset,
	     get_mb_elf_value('i', ei_data, &tmp, &shdr->sh_size), tmp.i);
...
static inline int read_elf_e_shentsize(struct elf32_hdr *e_hdr, char **buff)
{
	...
	(buff[l_count++] = e_shentsize.name, buff[l_count++] =
	 e_shentsize.pad_start);
...
static inline int read_elf_e_phnum(struct elf32_hdr *e_hdr, char **buff)
{
...
static inline int read_elf_e_phentsize(struct elf32_hdr *e_hdr, char **buff)
{
	...
	(buff[l_count++] = e_phentsize.name, buff[l_count++] =
	 e_phentsize.pad_start);
...
static inline int read_elf_e_ehsize(struct elf32_hdr *e_hdr, char **buff)
{
...
static inline int read_elf_e_phoff(struct elf32_hdr *e_hdr, char **buff)
{
...
static inline int read_elf_e_entry(struct elf32_hdr *e_hdr, char **buff)
{
...
static inline int read_elf_e_type(struct elf32_hdr *e_hdr, char **buff)
{
...
#define ET_PROC 6
	...
	get_mb_elf_value('s', e_hdr->e_ident[EI_DATA], &tmp, &e_hdr->e_type);
...
static inline int read_elf_magic(struct elf32_hdr *e_hdr, char **buff)
{
	static char val[VAL_MAX];
...
static inline int read_elf_class(struct elf32_hdr *e_hdr, char **buff)
{
	int l_count = 0;
...
static inline int read_elf_data(struct elf32_hdr *e_hdr, char **buff)
{
	int l_count = 0;
...
static inline int read_elf_version(struct elf32_hdr *e_hdr, char **buff)
{
	...
	(buff[l_count++] = ei_version.name, buff[l_count++] =
	 ei_version.pad_start);
...
static inline int read_elf_ident(struct elf32_hdr *e_hdr, char **buff)
{
	unsigned long long l_count = 0;
